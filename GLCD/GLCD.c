...
void LCD_DrawEmptyRectangle (uint16_t x_start, uint16_t y_start, uint16_t width, uint16_t height, uint16_t color) 
{
	uint16_t i = 0;
	while(i <= width){
		LCD_SetPoint(x_start + i, y_start, color); //draw the upper horizontal part of the rectangol
		LCD_SetPoint(x_start + i, y_start + height, color); //draw the lower horizontal part of the rectangol
		i++;
	}
	i=0;
	while(i <= height) {
		LCD_SetPoint(x_start, y_start + i, color); //draw the lefter vertical part of the rectangol
		LCD_SetPoint(x_start + width, y_start + i, color); //draw the righter vertical part of the rectangol
		i++;
	}
}
...
void LCD_DrawFullCircle(int x, int y, int r, int y_in, int y_fin, int color)
  //0 < y_in < 2r, 0 < y_fin < 2r
{
  int i = 0;
  int x0, x2, y1, y2, r2;
  ...
  for (i=0; i < (2 * r) - y_fin; i++)
  {
    if(y1 >= y-r+y_in){
      y2 = (r - i) * (r - i);
      x0 = (int) sqrt(r2 - y2);
      LCD_DrawLine(x - x0, y1, x + x0, y1, color);
    }  
    y1 = y1 + 1;
  }
}

void draw_square(int x, int y, int col, int off_x, int off_y, int factor) {
	int i, j;	
	for(i = factor*x + off_x; i < factor*x + off_x + factor; i++)
		for(j = factor*y + off_y; j < factor*y + off_y + factor; j++)
			LCD_SetPoint(i, j, col);
}
